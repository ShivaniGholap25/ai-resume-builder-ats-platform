...
export default function JobMatchPage() {
  ...
  const handleReset = () => { setJdResult(null); setComparison(null); setHasResume(false); };
      ...
      <AnimatePresence mode="wait">
        {!jdResult ? (
          <motion.div key="input" initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }}
            ...
            <JDInputPanel onAnalyze={handleAnalyze} onCompare={handleCompare} isLoading={isLoading} hasResume={hasResume} />
          </motion.div>
        ) : (
          <motion.div key="results" initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} className="space-y-6">
            ...
            {comparison && <MatchScorePanel comparison={comparison} />}
